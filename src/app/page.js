          ...
          <div className="relative max-w-[1400px] mx-auto px-4 md:px-8">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
              ...
              <div className="lg:col-span-7 flex flex-col items-center lg:items-start text-center lg:text-left space-y-6 lg:space-y-7">
                ...
                {/* Main Headline */}
                <h1
                  className="font-display text-[clamp(2.4rem,5vw,4.2rem)] font-bold tracking-tight text-white leading-[1.08] animate-fade-in-up"
                ...
                >
                  Choose from 25+ luxury wedding templates, edit live directly on screen, and get your custom shareable link — all at a flat <strong className="text-amber-300 font-semibold">₹299</strong> with no watermarks.
                </p>
                ...
                {/* Feature Chips / Trust Bar */}
                <div
                  className="pt-4 border-t border-white/10 grid grid-cols-2 sm:grid-cols-4 gap-3 w-full max-w-2xl text-left animate-fade-in-up"
                ...
                >
                  {[
                    ...
                    <div
                      key={idx}
                      className="p-3 rounded-xl bg-white/5 border border-white/10 hover:border-amber-400/30 hover:bg-white/10 transition-all duration-300 group"
                    >
                      <div className="flex items-center gap-2 font-semibold text-xs sm:text-sm text-stone-200 group-hover:text-white transition-colors">
                        {item.icon}
                        <span>{item.title}</span>
                      </div>
                      <div className="text-[11px] text-stone-400 mt-0.5 pl-6">{item.sub}</div>
                    </div>
                  ))}
                </div>
      ...
      <footer className="relative bg-[#061812] text-white border-t border-[#1B4332]/60 pt-16 pb-12 overflow-hidden">
        ...
        <div className="relative max-w-[1400px] mx-auto px-4 md:px-8">
          ...
          <div className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-stone-400 font-light">
            <div>
              © {new Date().getFullYear()} Web Invites. Handcrafted with ❤️ for unforgettable celebrations.
            </div>
            <button
              onClick={scrollToTop}
              ...
              <div className="relative flex-1 w-full bg-[#FAF8F5] overflow-hidden">
                <div className="absolute inset-0 overflow-y-auto hide-scrollbar [-webkit-overflow-scrolling:touch]">
                  <div style={{ containerType: 'inline-size', width: '100%', maxWidth: '100%' }}>
                    <Suspense fallback={<div className="flex items-center justify-center h-full text-stone-400 text-sm">Loading preview…</div>}>
                      <PreviewComponent
                        key={previewTemplate}

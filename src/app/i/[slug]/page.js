...
import templateImporters from '@/components/templates/importers';
...
export default async function InvitationPage({ params, searchParams, request }) {
  ...
  // Reads referrer_via cookie if present for attribution
  try {
    const via = typeof searchParams?.via === 'string' ? searchParams.via.trim() : '';
    // Cookie reads only in server components
    const ck = cookies();
  ...
  try {
    ...
    const ip = getClientIpForTracking(headerList);
    const ipHash = hashIp(ip);
    const userAgent = (headerList.get('user-agent') || '').slice(0, 200);
    const referrer = (headerList.get('referer') || '').slice(0, 500);
  ...
  const templateLoader = templateImporters[invitation.template_id] || templateImporters['standard-crimson'];
  const { default: TemplateComponent } = await templateLoader();
  ...
  // 24-Hour Grace Period after initial free tier publishing:
  // Hosts and guests enjoy a clean, ad-free experience for the first 24 hours.
  // During this period, only the "Go Ad-Free" CTA is available.
  // Once 24 hours elapse, sponsor ad units activate on the template.
  const publishTimestamp = invitation.paid_at
    ? new Date(invitation.paid_at).getTime()
    : (invitation.created_at ? new Date(invitation.created_at).getTime() : Date.now());
  const hoursSincePublish = (Date.now() - publishTimestamp) / (1000 * 60 * 60);
  const isInGracePeriod = isFreeTier && hoursSincePublish < 24;

  // Only serve 3rd-party ad scripts once the 24-hour grace period has completed
  const isAdSupported = isFreeTier && !isInGracePeriod && invitation.is_ad_supported !== false;
  ...
  return (
    ...
    >
      <Suspense fallback={<InvitationLoadingSkeleton />}>
        ...
        >
          {/* Top Sponsor Ribbon on Free Tier Invitations (After Grace Period) */}
          ...
          {/* Consent-Aware Ads — only shown when ad-supported after grace period */}
          {isAdSupported && (
            <ConsentAwareAds

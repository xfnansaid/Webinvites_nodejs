...
import GracePeriodBanner from '@/components/ads/GracePeriodBanner';
...
export default async function InvitationPage({ params, searchParams, request }) {
  ...
  // 2-Hour Grace Period after initial creation/publish:
  // Give hosts a clean, ad-free preview for the first 2 hours so they can verify and share
  // with close family without immediate ad clutter, while presenting a clear upgrade CTA.
  const createdTimestamp = invitation.created_at ? new Date(invitation.created_at).getTime() : Date.now();
  const hoursSinceCreation = (Date.now() - createdTimestamp) / (1000 * 60 * 60);
  const isInGracePeriod = isFreeTier && hoursSinceCreation < 2;

  // Only serve 3rd-party ad scripts once the 2-hour grace period has completed
  ...
  return (
    ...
    >
      <Suspense fallback={<InvitationLoadingSkeleton />}>
        ...
        >
          {/* 2-Hour Grace Period Banner on Fresh Free Tier Invitations */}
          {isInGracePeriod && (
            <GracePeriodBanner

...
export async function DELETE(request, { params }) {
  ...
  if (!isOwner && !isPhoneOwner && !isEmailOwner && !isServiceRoleConfigured()) {
    return NextResponse.json(
      { error: 'You can only delete invitations you created with this account.' },
      { status: 403 }
    );
  }
  ...
  if (deleteErr) {
    return NextResponse.json({ error: deleteErr.message || 'Failed to delete invitation' }, { status: 500 });
  }

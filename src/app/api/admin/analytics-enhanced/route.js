...
export async function GET(request) {
  ...
  const rl = rateLimit({ key: `admin-analytics-ext:${ip}`, limit: 30, windowMs: 60_000 });
  if (!rl.ok) {
  ...
  try {
    ...
    // ── 2. Peak Traffic Hours Heatmap & Recent Views ─────────
    ...
    const viewsLast24h = views.filter((v) => v.created_at >= twentyFourHoursAgo).length;
    const viewsLast7d = views.filter((v) => v.created_at >= sevenDaysAgo).length;
    ...
    // Busiest hours (top 5 with views > 0)
    ...
    // Fallback info for WhatsApp RSVP setup if table is not used
    const totalWithRsvpSetup = (allInvites || []).filter(
      (inv) => inv.template_data?.whatsappNumber || inv.template_data?.phone || inv.template_data?.whatsapp
    ).length;

    // ── 5. Device Trends ─────────────────────────────────────
    const deviceMap = { Mobile: 0, Desktop: 0, Tablet: 0 };
    for (const v of views) {
      const dev = categorizeDevice(v.user_agent);
    ...
    return NextResponse.json({
      ...
      peakTraffic: {
        ...
        dayNames,
        maxVal: maxHeatVal,
        peakHour: peakVal > 0 && peakHour !== null ? `${peakHour}:00` : null,
        peakDay: peakVal > 0 && peakDay !== null ? dayNames[peakDay] : null,
        peakVal,
        topHours,
        dailyTotals,
      },
    ...
    return NextResponse.json({ error: 'Failed to compute enhanced analytics.' }, { status: 500, headers: NO_CACHE_HEADERS });
  }
}

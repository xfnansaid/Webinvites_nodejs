...
export async function GET(request) {
  ...
  const rl = rateLimit({ key: `admin-reset-stats:${ip}`, limit: 60, windowMs: 60_000 });
  if (!rl.ok) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to retrieve stats for reset preview.' }, { status: 500, headers: NO_CACHE_HEADERS });
  }
}
...
export async function POST(request) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Server error processing analytics reset.' }, { status: 500, headers: NO_CACHE_HEADERS });
  }
}

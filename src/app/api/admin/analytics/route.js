...
export async function GET(request) {
  ...
  const rl = rateLimit({ key: `admin-analytics:${ip}`, limit: 60, windowMs: 60_000 });
  if (!rl.ok) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to aggregate analytics.' }, { status: 500, headers: NO_CACHE_HEADERS });
  }
}

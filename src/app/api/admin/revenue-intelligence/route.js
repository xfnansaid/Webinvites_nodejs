...
export async function GET(request) {
  ...
  const rl = rateLimit({ key: `admin-revenue-intel:${ip}`, limit: 30, windowMs: 60_000 });
  if (!rl.ok) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to compute revenue intelligence.' }, { status: 500, headers: NO_CACHE_HEADERS });
  }
}

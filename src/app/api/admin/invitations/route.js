...
export async function GET(request) {
  ...
  try {
    ...
    if (tier === 'free') {
      ...
      query = query.or('tier.eq.premium,tier.is.null');
    }
    ...
    // Apply Search Filter across the core guaranteed text columns
    if (search) {
      const cleanSearch = search.replace(/[%_,]/g, ' ').trim();
      if (cleanSearch) {
        query = query.or(
          `slug.ilike.%${cleanSearch}%,groom_name.ilike.%${cleanSearch}%,bride_name.ilike.%${cleanSearch}%,owner_email.ilike.%${cleanSearch}%`
        );
      }
    }
...
function formatResponse(rows, count, page, limit, expiryFilter) {
  ...
  const invitations = (rows || []).map((row) => {
    ...
    return {
      ...
      isAdSupported: !!row.is_ad_supported,

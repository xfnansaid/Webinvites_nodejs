...
export async function POST(request) {
  ...
  try {
    ...
    let slug = invitationId
      ...
      : generateSlug(resolvedGroomName, resolvedBrideName);
    ...
    const [, order] = await Promise.all([
      ...
      invitationId
        ...
        : supabaseServer
            .from('invitations')
            .select('slug')
            .ilike('slug', `${(slug || 'invitation')}%`)
            .then(({ data: existingSlugs }) => {
    ...
    if (invitationId) {
      // UPDATE: existing invitation (Republish / Remove Ads / Edit-and-repay flow)
      // IMPORTANT: Do NOT mark tier='premium' or is_ad_supported=false yet!
      // The invitation remains in its current tier until confirm-payment verifies the signature.
      const updateRow = {
        razorpay_order_id: order.id,
        groom_name: resolvedGroomName,
        bride_name: resolvedBrideName,

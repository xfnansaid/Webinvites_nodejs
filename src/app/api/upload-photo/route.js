...
export async function POST(request) {
  ...
  try {
    // SECURITY: Authentication FIRST — before any expensive work (buffer alloc,
    // sharp compression, form parsing for big files). Unauthenticated attackers
    // should not be able to DoS the server by uploading multi-MB payloads.
    const { user } = await resolveSupabaseUser(request);
    if (!user) {
      return NextResponse.json(
        { error: 'Please sign in to upload photos.', code: 'AUTH_REQUIRED' },
        { status: 401 }
      );
    }
    const folderId = user.id;

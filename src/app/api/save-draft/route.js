...
export async function POST(request) {
  ...
  try {
    ...
    if (invitationId) {
      ...
      //
      // CRITICAL: Do NOT re-touch anonymous_created_at / referred_by_slug on
      // UPDATE — those are set once on initial INSERT and must never be changed
      // (otherwise we'd reset the 4h deletion window / clobber attribution).
      ...
      // Tier-2 fallback: swallow 42703 errors for anonymous_created_at /
      // referred_by_slug columns that may not be migrated on live DB yet.
      // Payment/referral features will still work via cookie-only path.
      if (error && error.code === '42703') {
        const colMsg = String(error.message || '');
        if (/anonymous_created_at|referred_by_slug/.test(colMsg)) {
          const tier2Row = { ...baseRow };
          delete tier2Row.anonymous_created_at;
          delete tier2Row.referred_by_slug;
          const retry2 = await supabaseServer
            .from('invitations')
            .insert([{ ...tier2Row, slug }])
            .select()
            .single();
          data = retry2.data;
          error = retry2.error;

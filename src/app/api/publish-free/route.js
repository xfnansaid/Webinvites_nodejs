...
export async function POST(request) {
  ...
  try {
    ...
    if (!invitationId) {
      ...
      // If updating an existing invitation, ALWAYS verify ownership — regardless of
      // whether the user's invite list is empty. (Prevents IDOR where a user with
      // zero existing invitations could overwrite any other user's invitation.)
      const target = existingList.find((inv) => String(inv.id) === String(invitationId));
      let isOwner = !!target;
      if (!isOwner) {
        // Not in the first list — fetch the row directly and compare ownership fields
        const { data: checkDirect } = await supabaseServer
          .from('invitations')
          .select('id, owner_id, owner_email, owner_phone')
          .eq('id', invitationId)
          .maybeSingle();

        if (!checkDirect) {
          // Invitation doesn't exist — treat as forbidden (no enumeration signal)
          return NextResponse.json(
    ...
    // free invitations AFTER the insert. If count > 1 we lost a double-click /

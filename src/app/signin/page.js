...
function SignInContent() {
  ...
  const { user, session, loading, signInWithGoogle, oauthError, userAvatar } = useAuth();
  ...
  // If user is signed in: check if they came from clicking "Publish for free"
  // and publish immediately, directing them straight to their live published page.
  useEffect(() => {
    if (loading) return;
    if (!user) return;
    if (autoPublishStartedRef.current) return;

    const staged = peekStagedEditsForRestore();
    
    // CASE 1: User came from clicking "Publish for free" on a template
    if (staged && staged.formData && (staged.intent === 'free' || !staged.intent)) {
      ...
      (async () => {
        try {
          // Resolve the access token reliably (from session state or directly from Supabase)
          ...
          const payload = {
            ...
            ...(staged.draftId || staged.existingInvitationId
              ? { invitationId: staged.draftId || staged.existingInvitationId }
              : {}),
            ...(staged.tempOwnerToken ? { tempOwnerToken: staged.tempOwnerToken } : {}),
          };
          ...
          if (res.ok && data?.slug) {
            discardStagedEdits();
            consumeAuthRedirectNext();
            const target = `/i/${data.slug}?success=true`;
            try {
              router.replace(target);
            } catch {}
            // Reliable hard redirect guarantee
            setTimeout(() => {
            ...
            console.warn('[signin] Direct free publish returned error response:', data);
          }
        } catch (publishErr) {
          console.warn('[signin] Direct free publish threw error, falling back to template editor:', publishErr);
        }

        // Fallback to editor with preserved edits if server returned an error (e.g. limit reached)
        const fallbackDest = staged.returnTo || (staged.templateId ? `/create/${staged.templateId}` : '/dashboard');
        try {
          router.replace(fallbackDest);
        } catch {
          window.location.assign(fallbackDest);
        }
      })();
    ...
    // CASE 2: User came with paid intent -> route back to template editor to resume Razorpay
    if (staged && staged.intent === 'paid') {
      autoPublishStartedRef.current = true;
      setRedirecting(true);
      const dest = staged.returnTo || (staged.templateId ? `/create/${staged.templateId}` : '/dashboard');
      try {
        router.replace(dest);
      } catch {
    ...
    // CASE 3: Standard sign-in navigation (no active template publish)
    let destination = '/dashboard';
    const stashed = consumeAuthRedirectNext();
    if (stashed) {
      destination = stashed;
    } else if (queryNext) {
      destination = queryNext;
    } else if (staged?.returnTo) {
      destination = staged.returnTo;
    }
  ...
  }, [user, session, loading, queryNext, router]);
  ...
  const handleGoogleSignIn = async (e) => {
    ...
    setSubmitting(true);
    const staged = peekStagedEditsForRestore();
    const nextRelative = queryNext || staged?.returnTo || '/dashboard';
        ...
        <div className="relative z-10 flex flex-col justify-between w-full px-10 xl:px-14 py-10">
          ...
          <div className="flex-1 flex flex-col justify-center max-w-lg">
            ...
            <div className="flex items-center gap-6 mb-8">
              ...
              <div className="flex flex-col">
                <span className="text-2xl font-bold text-white font-display">Free</span>
                <span className="text-[11px] uppercase tracking-widest text-white/40 font-semibold">To Publish</span>
              </div>
              <div className="w-px h-10 bg-white/10" />
              <div className="flex flex-col">
                <span className="text-2xl font-bold text-white font-display">Instant</span>
                <span className="text-[11px] uppercase tracking-widest text-white/40 font-semibold">Live Link</span>
              </div>
            </div>
              ...
              <div className="relative bg-white rounded-3xl shadow-[0_30px_80px_rgba(15,56,44,0.1)] border border-stone-100/80 overflow-hidden">
                ...
                <div className="px-7 sm:px-8 pt-8 sm:pt-10 pb-5 sm:pb-6 text-center">
                  ...
                  <p className="text-[13px] text-[var(--ink-muted)] leading-relaxed max-w-sm mx-auto">
                    {publishingStaged
                      ? 'Taking your customized invitation live with a shareable WhatsApp link…'
                      : signedIn
                        ? 'You are signed in securely. Taking you to your destination…'
                  ...
                  {signedIn && (
                    <div className="space-y-4 mb-5">
                      <div className="rounded-2xl bg-[var(--emerald-light)]/60 border border-[var(--emerald-primary)]/10 px-4 py-4 text-[13px] text-[var(--emerald-dark)] leading-relaxed flex items-start gap-3">
                        {publishingStaged ? (
                          <Loader2 className="w-5 h-5 shrink-0 mt-0.5 text-[var(--emerald-primary)] animate-spin" />
                        ) : (
                          <CheckCircle2 className="w-5 h-5 shrink-0 mt-0.5 text-[var(--emerald-primary)]" />
                        )}
                        <div>
                          <div className="font-bold mb-0.5">
                            {publishingStaged ? 'Publishing your invitation…' : 'Signed in successfully'}
                          </div>
                          <div className="opacity-80">
                            {publishingStaged ? 'Taking you to your live page now…' : 'Taking you to your dashboard…'}
                          </div>
                        </div>
                      </div>
                      {!publishingStaged && (

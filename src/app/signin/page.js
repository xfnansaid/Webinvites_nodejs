...
function SignInContent() {
  ...
  // If user is already signed in: check if they came from clicking "Publish for free"
  ...
  useEffect(() => {
    ...
    if (staged && staged.formData && (staged.intent === 'free' || !staged.intent) && session?.access_token) {
      ...
      (async () => {
        try {
          ...
          console.warn('[signin] Direct free publish error, falling back to template editor:', publishErr);
        }

        // Fallback to editor if server returned an error (e.g. limit reached)
        const fallbackDest = staged.returnTo || '/dashboard';

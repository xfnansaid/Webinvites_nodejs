...
export default function AdminGuestEngagementTab({ refreshTrigger }) {
  ...
  const viewsSummary = data?.viewsSummary || {};
  const activity = healthData?.checks?.activity || {};

  const views24h = viewsSummary.viewsLast24h ?? activity.viewsLast24h ?? 0;
  const views7d = viewsSummary.viewsLast7d ?? activity.viewsLast7d ?? 0;
  const totalRsvps = rsvp?.total ?? data?.totalWithRsvpSetup ?? 0;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <div className="rounded-2xl bg-white/95 border border-white/80 p-4 sm:p-5 shadow-sm">
          ...
          <div className="text-[11px] text-gray-500 mt-1">{views7d.toLocaleString()} this week</div>
        </div>
        ...
        <div className="rounded-2xl bg-white/95 border border-white/80 p-4 sm:p-5 shadow-sm">
          ...
          <div className="font-display text-2xl sm:text-3xl font-extrabold text-amber-800 mt-2">
            {peak.peakHour ? peak.peakHour : '—'}
          </div>
          <div className="text-[11px] text-gray-500 mt-1">
            {peak.peakDay ? `Peak day: ${peak.peakDay}` : 'No visit spikes yet'}
          </div>
        </div>
        ...
        <div className="rounded-2xl bg-white/95 border border-white/80 p-4 sm:p-5 shadow-sm">
          ...
          <div className="text-[11px] text-gray-500 mt-1">
            {rsvp ? `${rsvp.breakdown?.attending || 0} attending` : 'WhatsApp RSVP active'}
          </div>
        </div>
      </div>

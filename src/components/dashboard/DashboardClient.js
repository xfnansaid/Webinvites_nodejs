...
function prettyStatus(isPaid, statusColumn, isOwnerDraftColumn, tier, isAdSupported, isActive) {
  if (isActive === false && isPaid) {
    return { label: 'Expired · Inactive', tone: 'red', icon: XCircle, isExpired: true };
  }
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-6 sm:pt-10 pb-24">
            ...
            >
              ...
              <div className="flex-1">
                ...
                <div className="font-bold text-sm sm:text-base text-[var(--ink)] leading-tight">
                  Browse 25+ templates → edit → publish free (with ads) or ₹399 ad-free
                </div>
              </div>
        ...
        {!hasAnyInvite ? (
          <div className="rounded-3xl border-2 border-dashed border-[var(--emerald-primary)]/20 bg-white/70 p-8 sm:p-10 text-center">
            ...
            <p className="text-sm sm:text-base text-[var(--ink-muted)] leading-relaxed mb-5 max-w-md mx-auto">
              ...
              invitation free (with ads) — or go ad-free for ₹399. Shareable on WhatsApp, SMS, or any social platform.
            </p>
            <Link
              href="/"
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
              {invites.map((inv) => {
                const status = prettyStatus(inv.is_paid, inv.status, !inv.razorpay_order_id, inv.tier, inv.is_ad_supported, inv.is_active);
                ...
                return (
                  <div
                    key={inv.id}
                    className={`group rounded-3xl border-2 shadow-[0_18px_40px_rgba(15,56,44,0.08)] overflow-hidden transition-all hover:shadow-[0_22px_60px_rgba(15,56,44,0.12)] ${status.isExpired
                        ? 'bg-gradient-to-br from-red-50/70 via-white to-red-50/30 border-red-100/70 opacity-75'
                        : isDraft
                    ...
                    <div className="relative bg-gradient-to-br from-[var(--emerald-primary)]/10 via-white to-[var(--champagne-500)]/10 h-32 sm:h-40 overflow-hidden">
                      ...
                      <div className="absolute top-3 left-3 flex items-center gap-1.5">
                        ...
                        {/* Expiry badge — hidden when status already shows 'Expired' */}
                        {!status.isExpired && (() => {
                          const expiry = getInviteExpiry(inv);
                          if (expiry.isExpired) {
                            return (
                              <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-red-100 text-red-800 ring-1 ring-red-200 text-[10px] sm:text-[11px] font-bold uppercase tracking-widest">
                          ...
                          if (expiry.daysRemaining !== null && expiry.tier === 'free') {
                            ...
                            return (
                              <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] sm:text-[11px] font-bold uppercase tracking-widest ring-1 ${isUrgent ? 'bg-red-100 text-red-800 ring-red-200' : 'bg-amber-100 text-amber-800 ring-amber-200'}`}>
                          ...
                          if (expiry.daysRemaining !== null && expiry.tier === 'premium') {
                            return (
                              <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-emerald-100 text-emerald-800 ring-1 ring-emerald-200 text-[10px] sm:text-[11px] font-bold uppercase tracking-widest">
                    ...
                    <div className="p-4 sm:p-5 space-y-3">
                      {/* Share URL row — only show for active published/paid invitations */}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {!isDraft && status.isExpired && (
                          <a
                            href={`/edit/${encodeURIComponent(inv.id)}`}
                            className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-2xl bg-amber-500 text-white font-bold text-xs sm:text-sm shadow-md shadow-amber-500/15 hover:bg-amber-600 active:scale-[0.98] transition-all"
                          >
                        ...
                        {!isDraft && !status.isExpired && status.isAdSupported && (
                          ...
                          >
                            <Sparkles className="w-3.5 h-3.5 text-amber-600" /> Upgrade to ₹399
                          </Link>
                        )}
                      </div>
                    </div>
        ...
        <div className="mt-10 sm:mt-12 text-center text-[11px] sm:text-xs text-[var(--ink-muted)] leading-relaxed max-w-lg mx-auto space-y-2">
          ...
          <p className="text-amber-700/80 font-semibold">
            ⏰ Free tier invitations expire 21 days after publishing. Upgrade to Premium (₹399) to keep your invitation live until 3 days after your event.
          </p>
        </div>

...
function isPremiumInvite(inv) {
  ...
  return Boolean(
    ...
    (inv.razorpay_payment_id && !String(inv.razorpay_payment_id).startsWith('free_')) ||
    (inv.razorpay_order_id &&
      (String(inv.razorpay_order_id).startsWith('admin_') || String(inv.razorpay_order_id).startsWith('order_')) &&
      inv.paid_at),
  );
}
            ...
            >
              ...
              <div className="flex-1">
                ...
                <div className="font-bold text-sm sm:text-base text-[var(--ink)] leading-tight">
                  Browse templates → publish 1 free invite with ads, or pay ₹399 to make any invite premium ad-free
                </div>
              </div>
        ...
        {!hasAnyInvite ? (
          <div className="rounded-3xl border-2 border-dashed border-[var(--emerald-primary)]/20 bg-white/70 p-8 sm:p-10 text-center">
            ...
            <p className="text-sm sm:text-base text-[var(--ink-muted)] leading-relaxed mb-5 max-w-md mx-auto">
              ...
              invitation free with ads. Each account gets 1 free template, and you can upgrade any invitation to premium ad-free for ₹399.
            </p>
            <Link
              href="/"

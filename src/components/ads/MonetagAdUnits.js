 ...
 *
 * Runs ONLY on free-tier invitation pages (/i/[slug]) to monetize guest interactions.
 * Never runs during checkout, editor, or dashboard.
 */
export function MonetagPopunderAd({ directLinkUrl, onclickZoneId, vignetteZoneId }) {
  ...
  useEffect(() => {
    ...
    // 3. Frequency-capped client fallback trigger (max 1 per session)

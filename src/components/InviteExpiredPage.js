...
export default function InviteExpiredPage({
  ...
  const [shareUrl, setShareUrl] = useState(() => (slug ? `${defaultBase}/i/${slugEnc}` : ''));

  useEffect(() => {
    if (typeof window !== 'undefined' && window.location.hostname && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
      setShareUrl(slug ? `${window.location.origin}/i/${slugEnc}` : '');
    }
  }, [slugEnc, slug]);
  ...
  const handleCopy = useCallback(async () => {
    const urlToCopy = shareUrl || (typeof window !== 'undefined' ? `${window.location.origin}/i/${slug}` : '');
    try {
  ...
  const handleWhatsApp = useCallback(() => {
    const urlToShare = shareUrl || (typeof window !== 'undefined' ? `${window.location.origin}/i/${slug}` : '');
      ...
      <div className="relative z-10 w-full max-w-lg">
        ...
        <div className="rounded-[2rem] bg-white/[0.04] backdrop-blur-xl border border-white/10 shadow-2xl overflow-hidden">
          ...
          <div className="p-6 sm:p-8 text-center">
            ...
            {isFreeTier && (
              <a
                href={`/checkout?slug=${encodeURIComponent(slug || '')}&invitationId=${encodeURIComponent(invitation?.id || '')}`}
            ...
            {/* Share link (always render matching container to prevent hydration mismatch) */}
            {slug && (
              <div className="mb-5">

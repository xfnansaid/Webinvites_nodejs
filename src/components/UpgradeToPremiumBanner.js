...
function isPremiumInvite(invitation) {
  ...
  return Boolean(
    ...
    (invitation.razorpay_payment_id && !String(invitation.razorpay_payment_id).startsWith('free_')) ||
    (invitation.razorpay_order_id &&
      (String(invitation.razorpay_order_id).startsWith('admin_') || String(invitation.razorpay_order_id).startsWith('order_')) &&
      invitation.paid_at),
  );
}
  ...
  const handleUpgrade = useCallback(async () => {
    ...
    try {
      ...
      const options = {
        ...
        currency: 'INR',
        name: 'WEB INVITES',
        description: `Upgrade to Premium — ${(data.groomName && data.brideName) ? `${data.groomName} & ${data.brideName}` : 'Wedding Invitation'}`,
        ...
        handler: async function (rzpResponse) {
          ...
          // Redirect to live page with success flag (always use production URL)
          const appBase = process.env.NEXT_PUBLIC_APP_URL || 'https://www.webinvites.shop';
          window.location.href = `${appBase}/i/${data.slug}?success=true&upgraded=true`;
        },
        modal: {
          ondismiss: () => {
            setLoading(false);
        ...
        prefill: {
          name: (data.groomName && data.brideName) ? `${data.groomName} & ${data.brideName}` : 'Wedding Invitation',
          contact: user?.phone || '',
        },
        theme: {
          color: '#0F382C',

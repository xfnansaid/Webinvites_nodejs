...
async function stageEditsAndRedirect({
  ...
  // Tier 2 (best-effort, max 1.2s timeout so OAuth is never delayed) — save draft in Supabase.
  try {
    const accessToken = session?.access_token || null;
    const headers = { 'Content-Type': 'application/json' };
    if (accessToken) headers.Authorization = `Bearer ${accessToken}`;
    
    const draftPromise = fetch('/api/save-draft', {
      method: 'POST',
      headers,
    ...
    const data = await Promise.race([draftPromise, timeoutPromise]);
    if (data && data?.ok && data.tempOwnerToken) {
      saveStagedEdits({
        ...stage,
...
export default function PaymentBanner({
  ...
  const handlePayPaidRef = useRef(null);
  const executeFreePublishRef = useRef(null);
  const onAfterSignInAutoPublishRef = useRef(null);
  const autoPublishTriggeredRef = useRef(false);

  // On mount (after sign-in redirect): auto-publish staged edits and redirect to live invitation page
  useEffect(() => {
    ...
    const staged = peekStagedEditsForRestore({ templateIdFilter: templateId || null });
    if (staged && (staged.templateId === templateId || !existingInvitationId)) {
      autoPublishTriggeredRef.current = true;
      const t = setTimeout(async () => {
        // Read from refs to avoid temporal dead zone / stale closure
        const onAfter = onAfterSignInAutoPublishRef.current;
        if (typeof onAfter === 'function') onAfter();
        if (staged.intent === 'paid') {
          const payPaidFn = handlePayPaidRef.current;
          if (typeof payPaidFn === 'function') {
            payPaidFn(staged.formData || null, staged.draftId || staged.existingInvitationId || null);
          }
        } else {
          // Intent is 'free' (or default): automatically publish free tier directly
          const freePublishFn = executeFreePublishRef.current;
          if (typeof freePublishFn === 'function') {
            await freePublishFn(
              staged.formData || null,
              staged.draftId || staged.existingInvitationId || null,
              staged.tempOwnerToken || null
            );
          }
        }
      }, 350);
  ...
  /**
   * Free Tier Publish execution helper — calls /api/publish-free and redirects to live page
   */
  const executeFreePublish = useCallback(async (overrideFormData = null, overrideExistingId = null, overrideToken = null) => {
    ...
    try {
      const payload = {
        ...
        ...(overrideToken ? { tempOwnerToken: overrideToken } : {}),
      };

      // Resolve authorization headers reliably
      ...
      if (!res.ok || data.error) {
        if (data?.code === 'FREE_TIER_LIMIT_REACHED') {
          setEntitlements((prev) => ({
            ...(prev || {}),
            canPublishFree: false,
            freePublishedCount: 3,
          }));
        }
        setLastError({
          ...
          hint:
            data?.code === 'FREE_TIER_LIMIT_REACHED'
              ? 'You have reached the free invitation limit. Please proceed to pay ₹399 below or manage your invitations from your Dashboard.'
              : data?.details || 'Check the details above, or publish ad-free for ₹399.',
          showPayOption: data?.code === 'FREE_TIER_LIMIT_REACHED',
        });
        return;
      }

      // Publish succeeded! Discard staged edits from localStorage.
      discardStagedEdits();

      const targetUrl = `/i/${data.slug}?success=true`;
      try {
      ...
      setTimeout(() => {
        if (typeof window !== 'undefined' && !window.location.pathname.startsWith('/i/')) {
          window.location.assign(targetUrl);
        }
      }, 500);
    } catch (err) {
    ...
    try {
      ...
      const options = {
        ...
        modal: {
          ondismiss: () => {
            setLoading(false);
            window.dispatchEvent(new Event('razorpay-close'));
          },
        },
        prefill: {
      ...
      window.dispatchEvent(new Event('razorpay-open'));
      rzp.open();
    } catch (err) {
  ...
  }, [user, router, templateId, formData, existingInvitationId, userPhone, authHeaders, signInWithGoogle, session]);

  /**
   * Choice modal action: "Publish for free (with ads)"
   */
  const handleFreePublishChoice = useCallback(async () => {
    // If client is not signed in: stage edits and redirect to Google Auth!
    if (!user) {
      setLoading(true);
      await stageEditsAndRedirect({
        formData,
        templateId,
        existingInvitationId,
        router,
    ...
    // Client is signed in: execute publish immediately
    setShowRewardedModal(false);
    await executeFreePublish();
  }, [user, formData, templateId, existingInvitationId, router, signInWithGoogle, session, executeFreePublish]);

  /**
   * Choice modal action: "Get premium for (ad-free) exp · ₹399"
   */
  const handleUpgradeToPaidChoice = useCallback(async () => {
    // If client is not signed in: stage edits and redirect to Google Auth with intent: 'paid'
    if (!user) {
      setLoading(true);
      await stageEditsAndRedirect({
        formData,
    ...
    // Client is signed in: launch Razorpay
    setShowRewardedModal(false);
    await handlePayPaid();
  }, [user, formData, templateId, existingInvitationId, router, signInWithGoogle, session, handlePayPaid]);
  ...
  const handlePay = useCallback(async (overrideFormData = null, overrideExistingId = null) => {
    ...
    // When user clicks the main Publish button, show the RewardedPublishModal (Choice Popup)
    if (canPublishFree) {
      setShowRewardedModal(true);
      return;
    }

    // For non-free-tier users (e.g. already used their free publish),
    // require sign-in before proceeding to paid checkout.
    if (!user) {
      stageEditsAndRedirect({
        formData: overrideFormData || formData,
        templateId,
  ...
  }, [user, isAdmin, handleAdminPublish, handleSaveEditsOnly, invitationAlreadyPaid, existingInvitationId, formData, templateId, router, signInWithGoogle, session, canPublishFree, handlePayPaid]);

  handlePayRef.current = handlePay;
  handlePayPaidRef.current = handlePayPaid;
  executeFreePublishRef.current = executeFreePublish;
  onAfterSignInAutoPublishRef.current = typeof onAfterSignInAutoPublish === 'function'
    ? onAfterSignInAutoPublish
    : null;

  const templateDisplayName = useMemo(() => {
    if (formData?.celebrantName) return `${formData.celebrantName}'s Birthday`;
    if (formData?.familyName) return `${formData.familyName}'s Celebration`;
    if (formData?.groomName && formData?.brideName) return `${formData.groomName} & ${formData.brideName}`;
  ...
  const buttonLabel = useMemo(() => {
    ...
    if (canPublishFree) return 'Publish for free (with ads)';
    if (!user) return 'Sign in & Publish (₹399)';
    return 'Publish for ₹399 · Ad-Free';
  ...
  const buttonIcon = useMemo(() => {
    ...
    if (canPublishFree) return <Sparkles className="w-5 h-5 text-amber-300" />;
  ...
  return (
    <>
                ...
                {lastError.showPayOption && (
                  <div className="mt-3 flex flex-wrap items-center gap-2.5">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white font-bold text-xs shadow-md transition-all active:scale-95"
                    >
                      <Sparkles className="w-3.5 h-3.5 text-amber-300" />
                      <span>Pay ₹399 &amp; Publish Now</span>
                    </button>
                    <Link
                      href="/dashboard"
                      className="text-xs font-semibold text-stone-700 hover:text-stone-900 underline ml-1"
                    >
                      Go to Dashboard to delete old invite →
                    </Link>
                  </div>
                )}
            ...
            <div className="flex items-center gap-3 sm:gap-4 text-center md:text-left w-full md:w-auto md:min-w-0">
              ...
              <div className="hidden md:flex md:max-w-none items-center gap-2">
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-2xl bg-[var(--champagne-500)]/10 text-[11px] font-bold text-[var(--champagne-700)] ring-1 ring-[var(--champagne-500)]/20">
                  {isAdmin ? (
                    ...
                    <>
                      ...
                      Free Tier Available · 36h Grace Period
                    </>
                  ) : (
                    <>
                      <ShieldCheck className="w-3.5 h-3.5" />
                ...
                {canPublishFree && user && !isAdmin && (
                  ...
                  >
                    Or go 100% ad-free (₹399) →
                  </button>
                )}
              </div>
            </div>

            <div className="flex flex-col items-center sm:items-end justify-center w-full md:w-auto gap-1">
              <button
                onClick={() => handlePay()}
                disabled={loading}
              ...
              {/* Text mention below button: go ad-free for 399 */}
              {canPublishFree && !invitationAlreadyPaid && !isAdmin && (
                <button
                  type="button"
                  disabled={loading}
                  onClick={() => handlePayPaid()}
                  className="text-xs font-semibold text-stone-600 hover:text-emerald-950 transition-colors flex items-center gap-1 group py-0.5 px-1 cursor-pointer disabled:opacity-50"
                >
                  <span className="text-stone-500">or</span>
                  <span className="underline underline-offset-2 font-bold text-emerald-800 group-hover:text-emerald-950">
                    go ad-free for ₹399
                  </span>
                  <ArrowRight className="w-3 h-3 text-emerald-700 transition-transform group-hover:translate-x-0.5" />
                </button>
              )}
            </div>
          </div>
      ...
      {/* Publish Choice Modal for Free Tier Publishing */}
      <RewardedPublishModal
        isOpen={showRewardedModal}
        onClose={() => setShowRewardedModal(false)}
        onRewardEarned={handleFreePublishChoice}
        onUpgradeToPaid={handleUpgradeToPaidChoice}
        templateTitle={templateDisplayName}
        loading={loading}
      />
    </>
  );

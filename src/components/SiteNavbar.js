...
export default function SiteNavbar({ variant = 'home' }) {
  ...
  useEffect(() => {
    ...
    const onDoc = (e) => {
      ...
      if (typeof popRef.current.contains(e.target)) return;
      setOpen(false);
    };
    const onKey = (e) => {
          ...
          <Link href="/" className="flex items-center gap-2.5 sm:gap-3 shrink-0 group min-w-0">
            ...
            <div className="flex flex-col leading-tight min-w-0">
              ...
              <span className="text-[10px] sm:text-[11px] text-[var(--ink-soft)] font-medium tracking-wide uppercase">
                ₹299 Flat · WhatsApp Ready
              </span>
            </div>
          </Link>

          {/* Single user-circle icon trigger (desktop + mobile share this element) */}
          <div className="relative" ref={popRef}>
            <button
              type="button"
              onClick={() => setOpen(v => !v)}
              className="group relative inline-flex h-10 w-10 sm:h-11 sm:w-11 items-center justify-center rounded-full bg-stone-50 hover:bg-[var(--emerald-light)]/50 ring-1 ring-stone-200 hover:ring-[var(--emerald-primary)]/20 transition-all active:scale-[0.98] shadow-sm"
            ...
            >
              {signedIn ? (
                userAvatar ? (
                  ...
                  <span className="inline-flex items-center justify-center h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-[var(--emerald-primary)] text-white text-[11px] font-bold">
                    {userInitials(user)}
                  </span>
                )
              ) : (
                <span aria-hidden className="text-[18px] sm:text-[20px] leading-none select-none">
                  👤
                </span>
              )}
              {/* Small indicator dot when signed in */}
              {signedIn && (
                <span className="absolute bottom-0.5 right-0.5 h-2.5 w-2.5 rounded-full bg-[var(--emerald-primary)] ring-2 ring-white" />
              )}
            </button>
            ...
            {open && (
              <div
                role="menu"
                className="absolute right-0 top-[calc(100%+10px)] z-50 w-[260px] sm:w-[280px] origin-top-right rounded-2xl border border-stone-200 bg-white shadow-[0_18px_50px_rgba(15,56,44,0.14)] overflow-hidden"
              ...
              >
                {signedIn ? (
                  ...
                  <div className="flex flex-col p-2">
                    ...
                    <button
                      onClick={() => setOpen(false)}
                      className="mx-2 mb-2 text-[12px] text-[var(--ink-soft)] hover:text-[var(--ink)] hover:bg-stone-50 rounded-lg py-1.5 transition-colors"
                    >
                      Continue browsing
                    </button>
                  </div>

...
export default function InvitationSuccessShell({
  ...
  // so every WhatsApp / copied share link carries attribution for the viral loop.
  const shareUrl = useMemo(() => {
    if (typeof window === 'undefined') return '';
    // In production, always use the canonical domain. In dev, use current origin.
    const base =
      process.env.NEXT_PUBLIC_APP_URL ||
      (window.location.hostname === 'localhost'
        ? 'https://www.webinvites.shop'
        : `${window.location.protocol}//${window.location.host}`);
    const cleanBase = String(base).replace(/\/$/, '');
    const slugEnc = encodeURIComponent(slug);
    return `${cleanBase}/i/${slugEnc}?via=${slugEnc}`;
  }, [slug]);
  ...
  return (
    <main className="min-h-screen relative">
      ...
      {/* ============================================================
          GLOBAL CSS: hide PREVIEW/DRAFT overlays when .force-live-hide-watermark
          class is applied. Always rendered (harmless when class absent) to avoid
      ...
      {/* ------- VIRAL LOOP CTA — guests only (hide from owner) -------
           Every recipient who views this invite is pre-qualified traffic.
           Sticky bottom bar with a direct link to create their own invite
           using the SAME TEMPLATE pre-selected (they loved this design). */}
      {!isOwner && !guestCtaDismissed && (
        <div
          className="fixed bottom-3 sm:bottom-6 left-1/2 -translate-x-1/2 z-[65] w-[96%] max-w-md"
          role="region"
          aria-label="Make your own invitation"
        >
          <div className="relative w-full rounded-2xl sm:rounded-3xl bg-gradient-to-br from-[var(--emerald-primary)] via-[var(--emerald-primary)] to-[#0a4e3b] text-white shadow-[0_22px_60px_rgba(15,56,44,0.35)] overflow-hidden border border-white/10">
            {/* Soft glow */}
            <div className="pointer-events-none absolute -top-10 -right-10 w-40 h-40 rounded-full bg-white/10 blur-2xl" aria-hidden="true" />
            <div className="pointer-events-none absolute -bottom-16 -left-10 w-48 h-48 rounded-full bg-emerald-300/10 blur-3xl" aria-hidden="true" />
            <button
              type="button"
              onClick={dismissGuestCta}
              className="absolute top-2 right-2 sm:top-3 sm:right-3 inline-flex items-center justify-center w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 text-white/80 hover:text-white transition-colors backdrop-blur-sm"
              aria-label="Dismiss this banner"
            >
              <XIcon className="w-4 h-4 sm:w-4.5 sm:h-4.5" />
            </button>
            <div className="px-4 sm:px-5 py-3.5 sm:py-4.5 flex flex-col gap-3">
              <div className="flex items-start gap-3">
                <div className="shrink-0 mt-0.5 inline-flex items-center justify-center w-10 h-10 sm:w-11 sm:h-11 rounded-2xl bg-white/15 backdrop-blur-sm ring-1 ring-white/20">
                  <PartyPopper className="w-5 h-5 sm:w-5.5 sm:h-5.5 text-white" />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5 mb-1">
                    <Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-amber-200 shrink-0" />
                    <p className="text-[11px] sm:text-[12px] font-bold uppercase tracking-widest text-amber-100/90">
                      You're Invited to Make Your Own
                    </p>
                  </div>
                  <p className="text-[13px] sm:text-[15px] leading-snug font-semibold text-white/95">
                    Loved this invitation? Create a beautiful digital invite for your event in 2 minutes — same template, pre-filled for you.
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2 sm:gap-2.5">
                <Link
                  ...
                  className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 sm:px-5 sm:py-3 rounded-xl sm:rounded-2xl bg-white text-[var(--emerald-dark)] hover:bg-emerald-50 active:scale-[0.985] shadow-[0_6px_14px_rgba(0,0,0,0.12)] transition-all font-bold text-[12px] sm:text-[13px]"
                >
                  <Sparkles className="w-4 h-4 sm:w-4.5 sm:h-4.5" />
                  Make My Invite — It's Free to Try

...
// Exact Required Editable Component
const Editable = ({ tag: Tag = "span", value, field, onEdit, editable = false, className = "", placeholder = "", multiline = false }) => {
  ...
  if (isEditing) {
    const common = {
      ...
      className: `outline-none bg-white/10 ring-2 ring-blue-400/60 rounded px-2 py-0.5 ${className}`,
      placeholder
    };
    return multiline ? <textarea {...common} rows={Math.max(2, String(value ?? "").split("\n").length)} /> : <input as={undefined} {...common} />;
  }

  return (
    <Tag onClick={() => setIsEditing(true)} className={`cursor-pointer outline-none ring-0 hover:ring-2 hover:ring-blue-400/40 rounded transition-all ${className}`} title="Click to edit">
...
export default function WeddingTemplate({ data = {}, isDraft = false, editable = false, onEdit }) {
  ...
  const toggleMusic = () => {
    ...
    if (isPlaying) {
      ...
      audioRef.current.play().then(() => {
        setIsPlaying(true);
        showToast("🎵 Playing background wedding music");
      }).catch(() => {
  ...
  return (
    <div style={{ containerType: 'inline-size', width: '100%', maxWidth: '100%', margin: 0, padding: 0 }}  className="max-w-md mx-auto w-full min-h-screen bg-[#e8dfd2] text-[#332f2b] font-['Montserrat',sans-serif] relative overflow-x-hidden shadow-2xl selection:bg-[#c5a059]/30">
      ...
      {/* Hidden Audio Player */}
      <audio ref={audioRef} loop preload="auto" src={currentData.musicUrl} />
      ...
      <motion.button 
        ...
        className="fixed bottom-6 right-5 z-[600] w-13 h-13 rounded-full bg-[#2a151b] border border-[#c5a059] shadow-2xl flex items-center justify-center text-[#f3e6c8] cursor-pointer"
      >
        <span className={`absolute -top-1.5 -right-0.5 text-xs text-[#c5a059] ${isPlaying ? 'animate-bounce' : 'opacity-0'}`}>
          ♪
        </span>
        {isPlaying ? (
          <Volume2 className="w-6 h-6 text-[#c5a059] animate-pulse" />
        ) : (
          <VolumeX className="w-6 h-6 text-[#c5a059]/80" />
        )}
      </motion.button>

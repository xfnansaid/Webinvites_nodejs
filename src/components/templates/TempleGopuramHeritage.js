...
const Editable = ({
  ...
  return (
    <Tag
      ...
      className={`${className} outline-none ring-2 ring-amber-400/50 rounded px-1 transition-all cursor-text bg-white/40 hover:bg-white/50 min-w-[20px] inline-block`}
...
// Distinct animation style for this template
const sacredRise = {
...
export default function TempleGopuramHeritage({
  data = {},
  onEdit = () => {},
  editable = false,
  className = '',
  ...
  const heroEventText =
    data.heroEventText || 'with the blessings of the divine invite you';
  const countdownTitle =
    data.countdownTitle || 'The Auspicious Muhurtham';
    ...
    >
      ...
      <section id="hero-section" className="relative w-full overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img
            src="https://i.pinimg.com/736x/94/3f/eb/943feb4f2e40b3354546af2989ab64ed.jpg"
            alt=""
            className="h-full w-full object-cover object-top"
            loading="eager"
          />
          <div className="absolute inset-0 bg-gradient-to-b from-white/5 via-transparent to-[#faf6ef]" />
        </div>
        ...
        <div className="relative z-10 mx-auto flex min-h-[94vh] max-w-lg flex-col items-center justify-end px-5 pb-40 pt-28 text-center sm:pb-44">
          ...
          >
            {/* Decorative top – inspired by hanging lamps */}
            <motion.div
              variants={sacredRise}
              custom={0}
            ...
            {/* ===== GROOM + PARENTS ===== */}
            <motion.div variants={sacredRise} custom={1}>
              <Editable
                tag="h1"
                value={groomName}
                field="groomName"
                onEdit={onEdit}
                editable={editable}
                className="text-[clamp(1.85rem,9.5cqw,2.5rem)] font-normal tracking-[0.06em] text-stone-800 break-words leading-[1.1]"
                placeholder="Groom Name"
              />
            </motion.div>

            <motion.div variants={sacredRise} custom={2}>
              <Editable
                tag="p"
                value={groomParents}
                field="groomParents"
            ...
            <motion.div
              variants={sacredRise}
              custom={3}
              className="my-4 flex items-center gap-2 text-amber-600"
            >
            ...
            {/* ===== BRIDE + PARENTS ===== */}
            <motion.div variants={sacredRise} custom={4}>
              <Editable
                tag="h1"
                value={brideName}
                field="brideName"
            ...
            <motion.div variants={sacredRise} custom={5}>
              <Editable
                tag="p"
                value={brideParents}
                field="brideParents"
            ...
            {/* Event text */}
            <motion.div variants={sacredRise} custom={6} className="mt-5">
              <Editable
                tag="p"
                value={heroEventText}
                field="heroEventText"
                ...
                placeholder="with the blessings of the divine invite you"
              />
            </motion.div>
            ...
            >
              <div className="flex items-center gap-2 rounded-full border border-amber-300/60 bg-white/75 px-4 py-1.5 backdrop-blur-sm">
      ...
      <div className="relative z-10 mx-auto max-w-lg px-5 pb-20">
        ...
        >
          <motion.div variants={sacredRise} custom={0}>
            <Editable
              tag="h2"
              ...
              onEdit={onEdit}
              editable={editable}
              className="mb-7 text-[11px] font-medium uppercase tracking-[0.28em] text-amber-800"
              placeholder="The Auspicious Muhurtham"
            />
          </motion.div>
          ...
          {isExpired ? (
            <motion.div
              variants={sacredRise}
              custom={1}
              className="rounded-2xl border border-amber-600/20 bg-amber-50/70 p-6 shadow-sm"
            >
              <div className="mb-2 flex items-center justify-center gap-2">
                ...
                <Editable
                  ...
                  onEdit={onEdit}
                  editable={editable}
                  className="text-lg font-medium tracking-wide text-amber-950 sm:text-xl"
              ...
              <Editable
                ...
                onEdit={onEdit}
                editable={editable}
                className="mx-auto max-w-sm text-xs leading-relaxed text-amber-900/70 sm:text-sm"
            ...
            {[
              ...
              <motion.div
                ...
                variants={softBloom}
                custom={i}
                className="flex flex-col items-center rounded-2xl border border-amber-200/70 bg-white py-3.5 shadow-sm"
              >
                <span className="text-xl font-medium tabular-nums text-stone-800 sm:text-2xl">
                  {String(item.value).padStart(2, '0')}
                </span>
                <span className="mt-1 text-[10px] uppercase tracking-wider text-amber-700">
                  {item.label}
                </span>
              </motion.div>
            ))}
        ...
        <motion.section
          ...
          viewport={{ once: true, margin: '-50px' }}
          variants={sacredRise}
          className="mt-12 rounded-3xl border border-amber-200/70 bg-white p-6 shadow-sm"
        >
          <motion.div
            variants={sacredRise}
            custom={0}
          ...
          >
            ...
            <span className="text-[10px] font-medium uppercase tracking-[0.25em] text-amber-800">
              Venue
            </span>
          </motion.div>
          ...
          <motion.div variants={sacredRise} custom={3}>
            <a
              href={canonicalMapUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="mt-5 flex w-full items-center justify-center gap-2 rounded-full bg-amber-800 py-2.5 text-[13px] font-medium tracking-wide text-white transition hover:bg-amber-900 active:scale-[0.98]"
            >
        ...
        >
          <motion.div
            variants={softBloom}
            className="mb-4 flex items-center justify-center rounded-full border border-amber-300/60 bg-white text-sm font-medium tracking-widest text-amber-900 shadow-sm"

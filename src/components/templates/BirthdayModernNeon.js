...
export default function BirthdayModernNeon({ data = {}, isDraft, editable, onEdit, onStyleChange, templateData, }) {
  ...
  const m = { ...D, ...data };
  const displayDay = data.eventDay || formatDayOfWeek(m.birthdayDate, m.eventDay || 'Friday');
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white font-sans" style={{ containerType: 'inline-size', width: '100%', maxWidth: '100%' }}>
      ...
      <section className="relative min-h-[85vh] flex items-center justify-center overflow-hidden px-4 py-16">
        ...
        <div className="relative z-10 text-center max-w-lg mx-auto space-y-5">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-violet-500/10 border border-violet-500/30 text-violet-300 text-xs font-semibold">
            <Sparkles className="w-3.5 h-3.5" />
            <Editable tag="span" value={m.heroTagline} field="heroTagline" onEdit={onEdit} editable={editable} placeholder="Tagline" />
          </div>
          <Editable tag="p" value={m.heroIntro} field="heroIntro" onEdit={onEdit} editable={editable} className="text-stone-400 text-xs tracking-[0.3em] uppercase" placeholder="You're invited to" />
          <Editable tag="h1" value={m.celebrantName} field="celebrantName" onEdit={onEdit} editable={editable} className="font-[Space_Grotesk] text-[clamp(2.2rem,11cqw,3.8rem)] font-black text-transparent bg-clip-text bg-gradient-to-r from-violet-400 via-fuchsia-400 to-cyan-400 leading-[1] break-words" placeholder="Name" />
          <div className="flex items-center justify-center gap-3">
            <span className="h-px w-10 bg-gradient-to-r from-transparent to-violet-500" />
            <Editable tag="span" value={`${m.age}th Birthday`} field="age" onEdit={onEdit} editable={editable} className="font-[Space_Grotesk] text-2xl font-bold text-cyan-300 tracking-wider uppercase" placeholder="30th Birthday" />
            <span className="h-px w-10 bg-gradient-to-l from-transparent to-violet-500" />
          </div>
          {m.partyTheme && <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-cyan-500/10 border border-cyan-500/30 text-cyan-300 text-xs font-bold uppercase tracking-wider"><Star className="w-3.5 h-3.5" /><Editable tag="span" value={m.partyTheme} field="partyTheme" onEdit={onEdit} editable={editable} placeholder="Theme" /></div>}
          <div className="flex items-center justify-center gap-3 text-stone-400 text-sm">
            ...
            <span>•</span>
            <Clock className="w-4 h-4 text-cyan-400" /><Editable tag="span" value={m.birthdayTime} field="birthdayTime" onEdit={onEdit} editable={editable} className="font-semibold" placeholder="Time" />
          </div>
        </div>
      </section>

      {/* Countdown */}
      <section className="py-12 px-4 bg-[#0e0e16]">
        <div className="max-w-lg mx-auto text-center space-y-5">
          <Editable tag="h2" value={m.countdownTitle} field="countdownTitle" onEdit={onEdit} editable={editable} className="font-[Space_Grotesk] text-xl font-bold text-violet-300 uppercase tracking-wider" placeholder="Countdown" />
          {expired ? (
            <div className="text-2xl font-bold text-violet-400 animate-pulse">Let&apos;s Party! 🎉</div>
          ) : (
            <div className="grid grid-cols-4 gap-3 max-w-sm mx-auto" role="timer" aria-live="polite" aria-label="Countdown timer">
              {[{l:'Days',v:tl.d},{l:'Hours',v:tl.h},{l:'Min',v:tl.m},{l:'Sec',v:tl.s}].map(i => (
                <div key={i.l} className="bg-white/5 border border-violet-500/20 rounded-2xl p-3 backdrop-blur-sm">
                  <div className="font-[Space_Grotesk] text-2xl sm:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-b from-violet-300 to-fuchsia-400">{i.v}</div>
          ...
          <div className="bg-white/5 border border-violet-500/15 rounded-3xl p-6 space-y-3">
            <div className="flex items-center justify-center gap-4">
              <div className="text-center">
                <Editable tag="div" value={displayDay} field="eventDay" onEdit={onEdit} editable={editable} className="font-[Space_Grotesk] text-2xl font-bold text-violet-300" placeholder="Day" />
                <div className="text-[10px] text-stone-500 uppercase">Day</div>
              </div>
              <div className="h-10 w-px bg-violet-500/20" />
              <div className="text-center">
                <Editable tag="div" value={m.birthdayDate} field="birthdayDate" onEdit={onEdit} editable={editable} className="font-[Space_Grotesk] text-2xl font-bold text-violet-300" placeholder="Date" />
                <div className="text-[10px] text-stone-500 uppercase">Date</div>
              </div>
            </div>

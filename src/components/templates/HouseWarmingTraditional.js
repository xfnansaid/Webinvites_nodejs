...
export default function HouseWarmingTraditional({ data = {}, isDraft, editable, onEdit }) {
  const m = { ...D, ...data };
    ...
    <div className="min-h-screen text-stone-800 font-sans">
      ...
      <section className="relative min-h-[85vh] flex items-center justify-center overflow-hidden px-4 py-16">
        <div className="absolute inset-0 pointer-events-none">
          <img src="https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=800&auto=format&fit=crop&q=80" alt="" className="absolute inset-0 w-full h-full object-cover" aria-hidden="true" />
          <div className="absolute inset-0 bg-gradient-to-b from-[#FFF8F0]/95 via-[#FFF5E8]/90 to-[#FFFAF5]/98" />
          <div className="absolute top-20 left-1/4 w-[400px] h-[400px] rounded-full bg-amber-200/30 blur-[100px]" />
          <div className="absolute bottom-10 right-1/4 w-[300px] h-[300px] rounded-full bg-orange-100/40 blur-[80px]" />
          <div className="absolute inset-0 opacity-[0.04]" style={{ backgroundImage: 'radial-gradient(circle, #92400e 1px, transparent 0)', backgroundSize: '20px 20px' }} />
        </div>
        <div className="relative z-10 text-center max-w-lg mx-auto space-y-5">
          ...
          <Editable tag="p" value={m.heroIntro} field="heroIntro" onEdit={onEdit} editable={editable} className="text-amber-700 text-xs tracking-[0.3em] uppercase font-bold" placeholder="You're cordially invited to" />
          <Editable tag="h1" value={m.familyName} field="familyName" onEdit={onEdit} editable={editable} className="font-[Cinzel] text-[clamp(2rem,6vw,3.5rem)] font-bold text-amber-800 leading-[1.1]" placeholder="Family Name" />
          <Editable tag="p" value={m.heroTagline} field="heroTagline" onEdit={onEdit} editable={editable} className="font-[Cormorant_Garamond] italic text-lg text-stone-600" placeholder="We're blessed to have a new home" />
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-100 border border-amber-200">
            <Key className="w-4 h-4 text-amber-600" />
            <Editable tag="span" value="Griha Pravesh & Housewarming" field="partyTheme" onEdit={onEdit} editable={editable} className="text-amber-700 text-xs font-bold uppercase tracking-wider" placeholder="Griha Pravesh & Housewarming" />
          </div>
          <div className="flex items-center justify-center gap-3 text-stone-500 text-sm pt-2">
            <Calendar className="w-4 h-4 text-amber-600" />
            <Editable tag="span" value={m.eventDate} field="eventDate" onEdit={onEdit} editable={editable} className="font-semibold" placeholder="Date" />
            <span>•</span>
      ...
      {/* Countdown */}
      <section className="py-12 px-4 bg-gradient-to-b from-[#FFF8F0] to-[#FFF2E0]">
        <div className="max-w-lg mx-auto text-center space-y-5">
          <Editable tag="h2" value={m.countdownTitle} field="countdownTitle" onEdit={onEdit} editable={editable} className="font-[Cinzel] text-xl font-bold text-amber-700" placeholder="Days Until the Housewarming" />
          {expired ? (
            <div className="space-y-2"><Home className="w-10 h-10 text-amber-600 mx-auto" /><p className="text-amber-700 font-bold">Welcome to our new home! 🏠</p></div>
          ) : (
            <div className="grid grid-cols-4 gap-3 max-w-sm mx-auto" role="timer" aria-live="polite" aria-label="Countdown timer">
              {[{l:'Days',v:tl.d},{l:'Hrs',v:tl.h},{l:'Min',v:tl.m},{l:'Sec',v:tl.s}].map(i=>(
                <div key={i.l} className="bg-white border border-amber-200 rounded-2xl p-3 shadow-sm">
                  <div className="font-[Cinzel] text-2xl font-bold text-amber-700">{i.v}</div>
      ...
      {/* Save the Date & Ceremony Schedule */}
        ...
        <div className="relative max-w-lg mx-auto space-y-4">
          ...
          <div className="bg-white rounded-3xl border border-amber-100 p-6 shadow-sm space-y-4">
            <div className="flex items-center justify-center gap-4">
              <div className="text-center"><div className="font-[Cinzel] text-2xl font-bold text-amber-700">{m.eventDay}</div><div className="text-[10px] text-stone-400 uppercase">Day</div></div>
              <div className="h-10 w-px bg-amber-100" />
              <div className="text-center"><div className="font-[Cinzel] text-2xl font-bold text-amber-700">{m.eventDate}</div><div className="text-[10px] text-stone-400 uppercase">Date</div></div>
            </div>

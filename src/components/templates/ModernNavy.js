...
function parseTargetDateTime(dateInput, timeInput) {
  ...
  // Text date e.g. "Saturday, June 17, 2028" or "June 17, 2028"
  if (rawDate) {
...
export default function WeddingInvitationTemplate({
  ...
  // Derive weddingDateShort if weddingDate is an ISO or parsable date and short version wasn't overridden
  if (data?.weddingDate && !data?.weddingDateShort) {
    const parsed = parseTargetDateTime(data.weddingDate, data.weddingTime || baseValues.weddingTime);
    if (parsed) {
      baseValues.weddingDateShort = parsed.toLocaleDateString('en-US', {
        month: 'long',
        day: 'numeric',
        year: 'numeric',
      });
  ...
  React.useEffect(() => {
    const updateCountdown = () => {
      const targetObj = parseTargetDateTime(
        values.weddingDate || values.targetDate,
        values.weddingTime
      );
      ...
      const distance = targetObj.getTime() - Date.now();

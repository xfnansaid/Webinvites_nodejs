...
const Editable = ({ tag: Tag = "span", value, field, onEdit, editable = false, className = "", placeholder = "", multiline = false }) => {
  ...
  return (
    <Tag
      ...
      className={`cursor-pointer outline-none ring-0 hover:ring-2 hover:ring-blue-400/40 rounded transition-all ${className}`}
      title="Click to edit"
    >
...
export default function WeddingInvitationTemplate({
  ...
  const toggleMusic = async () => {
    if (!audioRef.current || !values.musicUrl) {
      return;
    }
  ...
  return (
    ...
    >
      ...
      <motion.button
        ...
        className="fixed bottom-[26px] right-[22px] z-[100] flex h-[54px] w-[54px] items-center justify-center rounded-full border-2 border-[#c5a059] bg-[#3b4d66] text-[#c5a059] shadow-[0_8px_20px_rgba(0,0,0,0.25)]"
      >
        {isPlaying ? (
          <>
            <Volume2 size={24} />
            ...
            <motion.span
              ...
              className="absolute -right-0.5 -top-1.5 text-[0.8rem] text-[#c5a059]"
            >
              <Music size={14} />
            </motion.span>
          </>
